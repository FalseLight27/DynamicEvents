...
function EventController:Update() --check Player position every 7 seconds
	...
	event1Dist = calculateDistance (EventLocations.event1Center, playerPos) --check player distance from Event locations
																			--every 7 seconds
	...
	Script.SetTimer(7000, EventController.Update)																		
...
local success, resultOrErrorMessage = pcall(start)

if success then
  print("EventControlller.start executed successfully")
else
  print("Error in EventControlller.start:", resultOrErrorMessage)
  print("StackTrace:", debug.traceback())  -- Print a stack trace for more details
end

--]==]

--defines what actually happens when Spawn function is called
function EventController:Spawn(side, position, objective, troopType)
    ...
    spawnParams.properties.sharedSoulGuid = EventGuids[troopType][side]
    if side == EventConstants.cuman_side then
...
function EventController:SpawnLeader(side, position)
    ...
    else if side == EventConstants.rat_side then
        leader.actor:EquipClothingPreset("4a45929c-690e-b01a-95ef-f975bbf6b0bd")
	else if side == EventConstants.bandit_side then
		leader.actor:EquipClothingPreset("4a45929c-690e-b01a-95ef-f975bbf6b0bd")
    end
