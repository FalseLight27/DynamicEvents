...
function EventController:OnSpawn()
    -- needed for OnUpdate callback
    self:Activate(1)
end

function EventController:UpdateStatus()
    message = "<font color='#FF3333' size='28'>Cuman War Status\n\n</font>"
    message = message .. "<font color='#333333' size='23'>Boehemian Strength: " .. tostring(EventController.Rattay.strength) .. "\n"
    message = message .. "Boehemian Treasury: " .. tostring(EventController.Rattay.money) .. "\n"
    message = message .. "Cuman Strength: " .. tostring(EventController.Cuman.strength) .. "\n"
    message = message .. "Cuman Treasury: " .. tostring(EventController.Cuman.money) .. "\n</font>"
    Game.ShowTutorial(message, 20, false, true)
end

function EventController:OnDestroy()
    self:ResetBattle()
...
function EventController:DestroyMarshal()
...
function EventController:OnSave(table)
	...
	table.currentBattle.dist = 0
end

function EventController:OnLoad(table)
	...
	self.nextEventLocation = EventLocations[self.ignoreLocationIdx]
...
function EventController.ClearTroops(self)
...
function EventController:ResetBattle()
...
function EventController.RemoveCorpse(entity)
...
function EventController:CreateAITagPoint(location)
    local spawnParams = {}
...
function EventController:CheckEventLikelihood(position)
		--find way to get player position
		
	local ret = {}
    ret.x = position.x
    ret.y = position.y
    ret.z = position.z
    if dist < 100 and math.random(100) <= WarConstants.eventChance then
	
	self.Properties.controller:StartEvent()
end

--]==]

function EventController:StartEvent()

-- IF PLAYER IS NEAR ONE OF THE EVENT ZONES (CHECK DIST) AND EVENT LIKELIHOOD ABOVE THRESHOLD,
    ...
    --if self.inBattle == false and self.readyForNewBattle == true and self.nextBattleLocation ~= nil then
        ...
        --local spawnParams = {}
        --local locations = self.nextEventLocation.possibleSpawnPoints -- doesnt exist yet
		
		--local location = EventLocations[idx]
            --self.ignoreLocationIdx = idx
		
		
        --message = "<font color='#FF3333' size='28'>FALSELIGHT EVENT MOD ACTIVATED!\n\n</font>"
		...
		--self:SpawnLeader(EventConstants.cuman_side, self.currentEvent.locations.cuman)
...
function EventController:AssignQuest()
...
function EventController:Debrief(won)
...
function EventController:Debrief2(won)
    QuestSystem.CompleteObjective("quest_warmod", "startBattle")
    QuestSystem.CompleteObjective("quest_warmod", "finishBattle")
...
function EventController:Brief()
...
function EventController:Scout()
...
function EventController:DetermineVictor()
...
function EventController:KillSide(side)
...
function EventController:CheckDeaths()
...
function EventController:DetermineDifficultyText()
...
-- temp numbers
function EventController:DetermineDifficulty()
    if self.regionalInfluence > WarDifficulty.impossible then
...
function EventController:GenerateRandomKey(tbl)
    local keyset = {}
...
function EventController:GenerateEvent()
    if math.random(100) <= WarConstants.eventChance then
        ...
        --[==[ -- TURN THIS INTO AN EVENT TYPE DETERMINER
...
function EventController:ReportEvent()
...
function EventController:GenerateRandomKey(tbl, ignore)
...
function EventController:ResetStrengths()
    for k in pairs (self.currentBattle.strengthPerWave) do
        self.currentBattle.strengthPerWave[k] = nil
    end
    -- reset strengths
...
function EventController:ReadyForBattle()
...
function EventController:OffScreenBattle()
    self:Debrief2(false)
    self:ResetBattle()
    self.ClearTroops(self)
end

function EventController:OnUpdate(delta)
